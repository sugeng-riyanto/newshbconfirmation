...
def show_success_page():
    ...
    st.write("Thank you for submitting the form. Here is the progress data:")

    # Fetch data from SQLite database
    conn = get_db_connection()
    c = conn.cursor()
    c.execute('SELECT grade, student_name, timestamp FROM responses')
    rows = c.fetchall()
    conn.close()
    ...
    if not rows:
        ...
        df = pd.DataFrame(rows, columns=["Grade", "Student Name", "Timestamp"])
        st.write(df)
...
def show_admin_page():
    ...
    st.sidebar.write("Here is for school admin only. Regards.")
